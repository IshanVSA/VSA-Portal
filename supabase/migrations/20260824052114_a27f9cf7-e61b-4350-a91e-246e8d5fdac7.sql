...
BEGIN
  ...
  -- Supabase Realtime's subscription manager runs a read-only heartbeat that always
  -- ends in ROLLBACK. Count those so we can exclude them from the app-level number.
  IF to_regclass('extensions.pg_stat_statements') IS NOT NULL THEN
    BEGIN
      EXECUTE $q$
        SELECT COALESCE(sum(calls), 0)
          FROM extensions.pg_stat_statements
         WHERE query ILIKE '%pg_publication_tables%'
            OR query ILIKE '%realtime.subscription%'
      $q$ INTO rt_heartbeat;
