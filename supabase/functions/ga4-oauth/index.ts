...
// The shared callback routes provider="ga4" back into the GA4 property picker flow.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "authorize") {
      ...
      const state = btoa(JSON.stringify({ clinic_id: clinicId, origin: originUrl, provider: "ga4" }));
      const authUrl =
