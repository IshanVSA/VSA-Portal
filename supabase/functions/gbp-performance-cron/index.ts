// Daily cron: sync GBP Performance for every clinic that has connected GBP.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const list = creds || [];
    console.log(`GBP Performance cron: syncing ${list.length} clinics`);

    const results: any[] = [];
    for (const c of list) {
      try {
        const res = await fetch(`${SUPABASE_URL}/functions/v1/sync-gbp-performance`, {
          method: "POST",
