...
async function extractWithAi(pages: PageData[]) {
  ...
  const response = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 6000,
      system: [
        "You are a strict information extractor for a veterinary clinic website.",
        "CRITICAL GROUNDING RULES:",
        "1. Extract ONLY facts that appear verbatim (or as a direct, faithful paraphrase) in the provided page content.",
        "2. NEVER invent, infer, guess, generalize, embellish, or fill gaps from prior knowledge or typical clinic assumptions.",
        "3. If a field is not clearly stated on the website content provided, OMIT the field entirely. Do not use placeholders like 'N/A', 'unknown', empty strings, or generic filler.",
        "4. Do not paraphrase marketing tone into new claims. Do not merge or normalize service names beyond what's written. Do not translate.",
        "5. For doctors/team: only include people explicitly named on the site. Only include credentials/role/bio text that is present on the site.",
        "6. For services: only include services actually listed. Do not add adjacent services that clinics 'usually' offer.",
        "7. For brand colors/fonts/logo: only include values you can literally see in the HTML/CSS/inline styles provided (hex codes, font-family strings, <img> logo URLs). If not present, omit.",
        "8. For tagline, mission, values, tone: only quote/paraphrase directly stated wording on the site. Do not synthesize a mission or tone from vibes.",
        "9. page_summaries must summarize ONLY what is on that specific page — no cross-page inference, no assumptions.",
        "10. Prefer omission over uncertainty. Confidence 'high' only when nearly every included field is directly quoted from the site; 'low' when much had to be omitted.",
        "Every field you emit MUST be traceable to a specific substring in the provided content.",
      ].join(" "),
