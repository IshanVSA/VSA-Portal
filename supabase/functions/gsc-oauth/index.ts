// Google Search Console OAuth: authorize -> callback (list sites) -> save (separate fn) | disconnect
...
const REDIRECT_URI = `${SUPABASE_URL}/functions/v1/gsc-oauth?action=callback`;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "callback") {
      ...
      let sitesData: any;
      try { sitesData = JSON.parse(sitesText); } catch {
        return new Response(null, {
          status: 302, headers: { Location: `${redirectBase}/clinics/${clinic_id}?error=list_sites` },
        });
      }
      ...
      for (const s of sitesData.siteEntry || []) {
        // Only include sites the user actually has data access for
        if (["siteOwner", "siteFullUser", "siteRestrictedUser"].includes(s.permissionLevel)) {
          sites.push({ site_url: s.siteUrl, permission_level: s.permissionLevel });
        }
      }
