                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {userRole !== "admin" && (
                        <span className="text-xs text-muted-foreground">
                          {p.team_role || <span className="italic">Unassigned</span>}
                        </span>
                      )}
                    </div>
                    <div>
                      <p className="text-[10px] uppercase tracking-wide text-muted-foreground mb-1.5">Assigned Clinics</p>
                      {assignedClinics.length > 0 ? (
                        <div className="flex flex-wrap gap-1">
                          {assignedClinics.map((name, i) => (
                            <Badge key={i} variant="secondary" className="text-[11px] rounded-full">{name}</Badge>
                          ))}
                        </div>
                      ) : (
