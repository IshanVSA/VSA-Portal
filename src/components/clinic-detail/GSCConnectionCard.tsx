...
import { CheckCircle2, RefreshCw, Loader2, Unlink, Clock, CalendarClock, Search, Globe } from "lucide-react";
...
export function GSCConnectionCard({ clinicId, onRefresh }: Props) {
  ...
  const nextSync = useMemo(() => {
    const now = new Date();
    let next = setSeconds(setMinutes(setHours(now, 7), 35), 0);
  ...
  const fetchCreds = async () => {
    ...
    const { data } = await (supabase as any)
      ...
      .select("site_url, site_display_name, permission_level, last_sync_at, last_sync_status, last_sync_error")
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      const res = await supabase.functions.invoke("sync-gsc-data", {
        body: { clinic_id: clinicId },
        headers: { Authorization: `Bearer ${session?.access_token}` },
      });
      if (res.error) throw new Error(await extractEdgeFunctionError(res.error, res.data, "Search Console sync failed"));
  ...
  if (!hasCreds) {
    return (
      <IOSGroup header="Google Search Console">
        ...
        <IOSRow
          centered
          label={<span className="text-primary font-medium">Connect Search Console</span>}
  ...
  return (
    <IOSGroup header="Google Search Console">
      <IOSRow icon={<CheckCircle2 />} tone="green" label={creds?.site_display_name || "Connected"} sublabel={creds?.permission_level || "Active connection"} />
      {creds?.site_url && (
        <IOSRow icon={<Globe />} tone="gray" label="Site URL" value={<span className="font-mono text-[10px] break-all">{creds.site_url}</span>} />
      )}
      <IOSRow
        icon={<Clock />}
        tone="indigo"
