...
export default function AdminDashboard() {
  ...
  return (
    ...
    >
        ...
        <div className="rounded-2xl border border-border/60 bg-background/50 p-4 backdrop-blur-sm sm:p-5">
          <header className="mb-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={() => { setTasksDeptFilter(null); setTasksOpen(true); }}
                className="flex items-center gap-1 rounded-full bg-primary/10 px-2.5 py-1 text-[11px] font-semibold text-primary transition-colors hover:bg-primary/20"
              >
                View all tasks
                <ArrowUpRight className="h-3 w-3" />
                ...
                <div key={dept} className="min-w-0">
                  <div className="mb-2 flex items-center gap-2 border-b border-border/50 pb-1.5">
                    ...
                    <button
                      ...
                      className="flex items-center gap-0.5 rounded-full px-1.5 py-0.5 text-[10px] font-medium text-primary transition-colors hover:bg-primary/10"
                      aria-label={`View all ${cfg.label} tasks`}
                    >
                      View all
                      <ArrowUpRight className="h-3 w-3" />
                    </button>
      ...
      <OpenTasksList open={tasksOpen} onOpenChange={setTasksOpen} initialDepartment={tasksDeptFilter} />
