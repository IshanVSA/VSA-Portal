...
const departmentServices: Record<string, { label: string; services: string[] }> = {
  ...
  google_ads: { label: "Google Ads", services: ["Call Volume Issues", "Wrong Call Tracking", "Others"] },
  social_media: { label: "Social Media", services: ["Content Request", "Client Visit", "Bulk Uploads", "Special Promotion", "Boost", "Others"] },
};
...
const pageTitles: Record<string, string> = {
  ...
  "/employees": "Team Members", "/reports": "Reports", "/settings": "Settings", "/cron-monitor": "Cron Monitor", "/login-errors": "Login Errors", "/sub-accounts": "Sub Accounts",
};
  ...
  return (
    <div className="flex min-h-screen bg-background">
      ...
      <main className={cn("flex-1 min-w-0 flex flex-col", collapsed ? "lg:pl-[68px]" : "lg:pl-[260px]")}>
        ...
        >
          {/* Left: menu + breadcrumb (fixed, never scrolls) */}
          <div className="flex items-center gap-2 lg:gap-4 pl-3 sm:pl-4 lg:pl-8 pr-2 shrink-0">
            <button
            ...
            {currentPageTitle && !showClinicSelector && (
              <span className="lg:hidden font-semibold text-foreground text-sm truncate min-w-0">
                {currentPageTitle}
              </span>
            )}
          </div>
